    ...
    while (fgets(line, sizeof(line), in)) {
        ...
        if (mnemonic[0] == '.') {
            if (strcmp(mnemonic, ".word") == 0) {
                ...
                // .space N  -  N bayt rezervi (word'e yukari yuvarla)
                char* val_s = strtok(NULL, " ,\t\r\n");
                int32_t n = (int32_t)strtol(val_s, NULL, 0);
                int32_t bytes = ((n + 3) / 4) * 4;
                if (strcmp(current_section, "TEXT") == 0) current_text_pc += bytes;
                else current_data_pc += bytes;
            }
            continue;
        }
    ...
    while (fgets(line, sizeof(line), in)) {
        ...
        if (mnemonic[0] == '.') {
            if (strcmp(mnemonic, ".word") == 0) {
                ...
                // .space N  -  N bayt rezervi (sifirlarla doldurulur).
                // Word hizalamasi icin yukari yuvarlanir.
                char* val_s = strtok(NULL, " ,\t\r\n");
                int32_t n = (int32_t)strtol(val_s, NULL, 0);
                int32_t words = (n + 3) / 4;
                for (int wi = 0; wi < words; wi++) {
                    if (strcmp(current_section, "TEXT") == 0) {
                        text_section[text_count++] = 0;
                        current_text_pc += 4;
                    } else {
                        data_section[data_count++] = 0;
                        current_data_pc += 4;
                    }
                }
            }
